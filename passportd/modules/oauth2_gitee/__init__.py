...
__plugin_name__ = "oauth2_gitee"
__version__ = "0.1.0"
__author__ = "staugur"
__description__ = "Login with Gitee OAuth2"
__license__ = "Apache-2.0"
__oauth2_provider__ = True
__oauth2_name__ = "Gitee"
__state__ = (
    "enabled"
    if config.get("GITEE_CLIENT_ID") and config.get("GITEE_CLIENT_SECRET")
    else "disabled"
)

bp = Blueprint(__plugin_name__, __plugin_name__)

gitee: FlaskOAuth2App = OAuthClient.register(
    name="gitee",
...
def login():
    ...
    return gitee.authorize_redirect(url_for(".authorized", _external=True), **kwargs)
...
def register():
    ...
    :returns: 包含 ``bep`` 键的字典，指定蓝图和 URL 前缀 ``/oauth2/gitee``
    """
    return dict(
        bep=dict(blueprint=bp, prefix="/oauth2/gitee"),
    )
